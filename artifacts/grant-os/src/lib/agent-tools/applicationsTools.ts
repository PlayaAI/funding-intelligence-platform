  ...
  return [
    {
      ...
      inputSchema: z.object({ limit: z.number().int().positive().max(200).optional(), grantId: z.string().optional() }),
      ...
      async execute({ limit, grantId }) {
        ...
        return { items: limit ? applications.slice(0, limit) : applications, total: applications.length };
      },
    },
    {
