...
import { buildGrantPacket, sortGrantsForSearch, stripDocumentContent } from "./builders";
  ...
  return [
    ...
    },
    {
      ...
      async execute({ grantId, includeExtractedText }) {
        ...
        return { grant, documents: includeExtractedText ? documents : documents.map(stripDocumentContent) };
      },
    },
    {
    ...
    },
    {
      ...
      permissionLevel: "read",
      inputSchema: z.object({ grantId: z.string().min(1), compact: z.boolean().optional() }),
      dryRunSupported: false,
      ...
      touchesRealDb: true,
      async execute({ grantId, compact }) {
        const grant = await repository.getGrant(grantId);
        if (!grant) throw makeToolError("grant_not_found", `Grant ${grantId} was not found.`);
        const funder = grant.funder_id ? await repository.getFunder(grant.funder_id) : null;
        return buildGrantPacket(repository, grant, funder, compact ?? true);
      },
    },
  ];
}
