...
type PackageType = "project" | "grant" | "application" | "funder";
...
export async function exportProjectPackage(projectId: string, filenameHint: string) {
  ...
  const [grants, proofItems, applications, tasks, agentNotes, agentReports] = await Promise.all([
  ...
  const payload = packageBase("project", { project, grants, proof_items: proofItems, applications, tasks, agent_notes: agentNotes, agent_reports: agentReports });
...
export async function exportGrantPackage(grantId: string, filenameHint: string) {
  ...
  const [funder, project, applications, tasks, agentNotes, agentReports] = await Promise.all([
  ...
  const payload = packageBase("grant", { grant, funder, project, applications, tasks, proof_items: proofItems, agent_notes: agentNotes, agent_reports: agentReports });
...
export async function exportApplicationPackage(applicationId: string, filenameHint: string) {
  ...
  const [grant, project, questions, requiredDocs, tasks, agentNotes, agentReports] = await Promise.all([
  ...
  const payload = packageBase("application", { application, grant, project, questions, required_documents: requiredDocs, tasks, agent_notes: agentNotes, agent_reports: agentReports });
...
export async function exportFunderPackage(funderId: string, filenameHint: string) {
  ...
  const [grants, peerFundingRecords, agentNotes] = await Promise.all([
  ...
  const payload = packageBase("funder", { funder, grants, peer_funding_records: peerFundingRecords, agent_notes: agentNotes, agent_reports: [] });
